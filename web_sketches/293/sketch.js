// mesh with mouse-driven offset + synthesized-amplitude xstep/ystep stretching.
// Original drove the stretch from mic — substituted with noise/sine signal.
...
function draw() {
  ...
  const fakeAmp = 0.05 + 0.04 * sin(frameCount * 0.04) + 0.03 * noise(frameCount * 0.02);
  smoothSum += (fakeAmp - smoothSum) * 0.5;
  const s = smoothSum * width * 5;
  const xstep = s;
  const ystep = s;
