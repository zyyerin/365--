class Mesh {
  ...
  Mesh(int display_x, int display_y) {
    noc_x = 20;
    noc_y = 20;
    noc = noc_x * noc_y;
  ...
  void display(float cubeSize,float cubeStrokeWeight, color clr, color connectionC) {
  strokeWeight(cubeStrokeWeight);
    pos = new PVector(0, 0);

    float mx = map(mouseX, 0, width, 0, 200);
    float my = 0;
    offset = new PVector(mx, my);

    for (int i = 0; i < cubes.length; i++) {
      float cubeS_ = cubeSize+mx+my;
  ...
  void dataUpdate() {
    //cubeS += 0.003;
    //offset.normalize();
    xstep = map(mouseX, 0, width, 50, 20);
    ystep = map(mouseY, 0, height, 50, 20);
    //xstep = 30;
    //ystep = 30;
    //cubeS = 5;
